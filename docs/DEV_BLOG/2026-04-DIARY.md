...
## 2026-04-17 (SESSION CURRENT): Fixed replay header writer to use explicit UTF-16 serialization

Continued replay triage after confirming GameInfo bytes existed in `.rep` files while parser still reported `invalid GameInfo len=0`.

What was fixed:
- identified write/read mismatch in replay header Unicode fields:
  - reader already consumes UTF-16 fixed-width units,
  - writer still used wide `writeFormat(L"%s")` which emits host `wchar_t` width.
- added explicit UTF-16 writer helper in both Recorder implementations:
  - `GeneralsMD/Code/GameEngine/Source/Common/Recorder.cpp`
  - `Generals/Code/GameEngine/Source/Common/Recorder.cpp`
- replaced header Unicode writes (`replayName`, `versionString`, `versionTimeString`) with fixed 2-byte unit serialization plus UTF-16 terminator.

Validation status:
- static diagnostics report no errors in modified Recorder files.
- full macOS build currently recompiling after clearing stale PCH files caused by CommandLineTools SDK mtime change.

Result:
- replay header layout is now deterministic for Unicode fields across Windows/macOS/Linux and aligned with retail-compatible UTF-16 expectations.

## 2026-04-17 (SESSION CURRENT): Analyzed replay header data structure alignment issue and prepared main branch merge

Analyzed newly recorded Skirmish replays (3.6M each, macOS) and identified root cause of `invalid GameInfo len=0` parse failures: **64-bit vs 32-bit alignment mismatch in header struct**.

What was discovered:
- binary hex dump of `macos_skirmish.rep` shows GameInfo string correctly written to file with proper ASCII content.
- parser reports `len=0` on same file, indicating **read position is misaligned** before attempting to parse GameInfo.
- root cause analysis: `sizeof(UnsignedInt)` can vary (4 or 8 bytes) between 32/64-bit builds.
  - Three version info fields written as `UnsignedInt` before GameInfo: `versionNumber`, `exeCRC`, `iniCRC`
  - if any of these are 8 bytes in 64-bit but read as 4 bytes, reader skips past null terminator into garbage.
- prepared to merge origin/main (added Flatpak CI job, fixed INI parser Linux break, additional session notes).

Validation:
- confirmed replay binary structure via hexdump - GameInfo content definitely written.
- identified specific write/read asymmetry in Recorder.cpp startRecording vs readReplayHeader.
- merge of origin/main completed with conflict resolution (kept both diary entries, accepted flatpak CI changes).

Next steps:
- verify `sizeof(UnsignedInt)` macros in both write/read paths (likely 32-bit definition vs 64-bit runtime)
- confirm struct packing rules align between write and read
- test with explicit-size types (`uint32_t`) if needed.
