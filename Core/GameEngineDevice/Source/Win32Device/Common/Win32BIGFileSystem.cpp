...
Bool Win32BIGFileSystem::loadOneArchiveFile(const AsciiString& archivePath, Bool overwrite)
{
	...
	DEBUG_LOG(("Win32BIGFileSystem::loadBigFilesFromDirectory - loading %s into the directory tree.", archivePath.str()));
	loadIntoDirectoryTree(archiveFile, overwrite);
	m_archiveFileMap[archivePath] = archiveFile;
	DEBUG_LOG(("Win32BIGFileSystem::loadBigFilesFromDirectory - %s inserted into the archive file map.", archivePath.str()));
	return TRUE;
}
...
Bool Win32BIGFileSystem::loadBigFilesFromDirectory(AsciiString dir, AsciiString fileMask, Bool overwrite, Bool allowPatchPrecedence) {
