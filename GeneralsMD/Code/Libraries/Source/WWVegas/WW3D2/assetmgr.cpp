...
RenderObjClass * WW3DAssetManager::Create_Render_Obj(const char * name)
{
	...
	if (name == NULL || name[0] == '\0') {
		...
		// Return a safe Null render object instance to keep engine progressing
		// Note: returning NULL here would propagate failure paths widely; Null keeps UI stable
	...
	if (WW3D_Load_On_Demand && proto == NULL) {	// If we didn't find one, try to load on demand
		...
		if (mesh_name != NULL) {
			::lstrcpyn (filename, name, ((uintptr_t)mesh_name) - ((uintptr_t)name) + 1);
	...
	if (proto == NULL) {
		...
		// Fallback to a Null render object to avoid hard crashes in downstream code paths
