...
**Changes Required**:
- Replace all file operations → `std::filesystem`
- Replace all registry calls → INI file parsing (already exists!)
- Replace all hardcoded paths → `SDL_GetPrefPath()`
- Replace all directory operations → `std::filesystem::create_directories()`
...
- [ ] Paths resolve correctly (`~` on Mac/Linux, `%APPDATA%` on Windows → all → `SDL_GetPrefPath()`)
