...
int main(int argc, char* argv[])
{
	...
	strcpy(debugLogName, strrchr(configName, GET_PATH_SEPARATOR()));
...
void myChdir(char* path)
{
	...
	if (filepath[strlen(filepath) - 1] == GET_PATH_SEPARATOR())
	{
	...
	if (!_chdrive(abc))
	{
		abc = chdir(filepath);  // Will fail with ending GET_PATH_SEPARATOR()
	}
