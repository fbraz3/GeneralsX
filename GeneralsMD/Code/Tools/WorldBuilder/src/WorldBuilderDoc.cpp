...
public:
	...
	virtual Int write(const void* pData, Int numBytes) {
		...
		try {
			...
			DEBUG_LOG(("write - Exception caught: %s", e.what()));
		}
		catch (...) {
			DEBUG_LOG(("write - Unknown exception caught"));
		...
		virtual void flush(void) {
			while (m_cachedChunks.size() != 0)//!m_cachedChunks.empty())
			{
				...
				try {
					...
					DEBUG_LOG(("flush - Exception caught: %s", e.what()));
				}
				catch (...) {
					DEBUG_LOG(("flush - Unknown exception caught"));
					delete[] c.pData;
					m_totalBytes -= c.size;
				}
			}
				...
				if (m_heightMap) try {
					...
					try {
						...
						BOOL CWorldBuilderDoc::OnOpenDocument(LPCTSTR lpszPathName)
						{
							...
							const char* lastSep = s.reverseFindPathSeparator();
							if (lastSep != NULL)
							{
...
#define DUMP_RAW_DICTS
						...
						void CWorldBuilderDoc::OnDumpDocToText(void)
						{
							...
							try {
								...
								if (char* pEnd = strrchr(curbuf, GET_PATH_SEPARATOR()))
								{
									*(pEnd + 1) = 0;
								}
