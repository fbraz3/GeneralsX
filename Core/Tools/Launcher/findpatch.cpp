...
bit8 Get_App_Dir(OUT char* filename, int maxlen, ConfigFile& config, int index)
{
  ...
  // Find the last GET_PATH_SEPARATOR() in a string and put a 0 after it
  //  If you only put a directory in the InstallPath key instead of a full
  //  path to a file, you better end the directory with a trailing GET_PATH_SEPARATOR()!!!
  char* cptr = gamePath;
  char* tempPtr;
  while ((tempPtr = strchr(cptr, GET_PATH_SEPARATOR())) != NULL)
    cptr = tempPtr + 1;
  if (cptr)
    *cptr = 0;
