...
## 2026-04-09 (SESSION CURRENT): Flatpak runtime strategy follow-up (Track A/Track B bootstrap)

Moved from diagnosis-only state to execution-ready next steps for runtime compatibility work.

**Implemented now:**
- Fixed a real bug in Flatpak wrapper scripts: removed unreachable environment exports placed after `exec`.
- Added an explicit experimental runtime mode in both wrappers:
  - `GENERALSX_FLATPAK_RUNTIME_MODE=stock` (default)
  - `GENERALSX_FLATPAK_RUNTIME_MODE=vendor-xcb` (prepends `/app/lib` and enables software Vulkan knobs)
- Added reproducible diagnostics helper script:
  - `scripts/qa/smoke/collect-flatpak-vulkan-wsi-report.sh`
  - Collects host/runtime metadata + Vulkan loader logs + symbol check for `xcb_dri3_import_syncobj_checked`
- Added active-work support docs for the next phase:
  - `docs/WORKDIR/support/FLATPAK_RUNTIME_PATCH_STRATEGY_2026-04.md`
  - `docs/WORKDIR/support/FREEDESKTOP_SDK_VULKAN_WSI_ISSUE_TEMPLATE.md`
- Updated `scripts/README.md` with the new QA diagnostics script.

**Validation:**
- Script syntax check passed (`bash -n`).
- Diagnostics script execution succeeded and generated a timestamped report under `logs/`.

**Outcome:**
- Repository now contains an execution path for app-level validation and an upstream-ready issue package.
- Next action can be either runtime custom branch bootstrap or immediate issue submission to Freedesktop SDK with generated logs.
...
- Created complete Flatpak infrastructure:
  - Dual manifests for GeneralsX and GeneralsXZH (com.generals.*.yml)
...
- `flatpak/com.generals.GeneralsX*.yml` - Complete manifests
- `flatpak/com.generals.GeneralsX*.desktop` - Desktop entries
- `flatpak/com.generals.GeneralsX*.metainfo.xml` - AppStream metadata
