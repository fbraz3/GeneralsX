...
LONG SDL2_RegistryPathToINIPath(const char* registry_path, char* output_path, unsigned long output_size)
{
    ...
    const char* last_backslash = strrchr(registry_path, GET_PATH_SEPARATOR());
    if (last_backslash) {
        p = last_backslash + 1;
    }
    else if (strchr(registry_path, GET_PATH_SEPARATOR())) {
        p = strrchr(registry_path, GET_PATH_SEPARATOR()) + 1;
    }
