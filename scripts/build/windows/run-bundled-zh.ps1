# GeneralsX Windows bundled runtime launcher for Zero Hour.
# GeneralsX @build BenderAI 02/04/2026 Run bundled GeneralsXZH with local runtime environment setup.
...
$exePath = Join-Path $installDir "GeneralsXZH.exe"
...
$env:CNC_GENERALS_PATH = (Join-Path $runtimeRoot "Generals")
$env:CNC_GENERALS_ZH_PATH = $installDir
if (-not $env:CNC_GENERALS_INSTALLPATH) {
    $env:CNC_GENERALS_INSTALLPATH = $env:CNC_GENERALS_PATH
}
$env:CNC_ZH_INSTALLPATH = $installDir
