...
## 2026-05-18: Windows64 execution-doc refactor and per-phase references

Refined Windows64 workstream docs to keep them execution-first and easier for
LLM-guided implementation.

What was done:
- converted pending phase descriptions to execution-oriented wording with clear
	implementation outcomes and done criteria.
- updated task docs to include `Implementation Reading (LLM)` sections directly
	inside each phase document, instead of relying on a separate references area.
- embedded task references directly in each phase block of the main execution
	strategy document.
- removed the separate child-task list section from the main strategy in favor
	of phase-local references.

Validation:
- diagnostics are clean for all updated markdown planning files.
- phase/task cross-linking remains consistent after the refactor.
