...
function(GetGitState _working_dir)
    ...
    # Allow passing override environment variables from CI or sandboxed environments
    if(DEFINED ENV{GENERALS_GIT_OVERRIDE_TSTAMP} OR DEFINED ENV{GENERALS_GIT_OVERRIDE_TAG})
        set(ENV{GIT_IS_DIRTY} "false")
        if(DEFINED ENV{GENERALS_GIT_OVERRIDE_IS_DIRTY})
            set(ENV{GIT_IS_DIRTY} "$ENV{GENERALS_GIT_OVERRIDE_IS_DIRTY}")
        endif()
        ...
        if(DEFINED ENV{GENERALS_GIT_OVERRIDE_TSTAMP})
            set(ENV{GIT_COMMIT_TSTAMP} "$ENV{GENERALS_GIT_OVERRIDE_TSTAMP}")
        endif()
        set(ENV{GIT_HEAD_SHORT_SHA1} "00000000")
        set(ENV{GIT_REV_LIST_COUNT} "0")
        set(ENV{GIT_TAG} "")
        if(DEFINED ENV{GENERALS_GIT_OVERRIDE_TAG})
            set(ENV{GIT_TAG} "$ENV{GENERALS_GIT_OVERRIDE_TAG}")
        endif()
        return()
    endif()
