...
void CampaignManager::init(void)
{
	...
	AsciiString campaignPath;
	campaignPath.concat("Data");
	campaignPath.concat(GET_PATH_SEPARATOR());
	campaignPath.concat("INI");
	campaignPath.concat(GET_PATH_SEPARATOR());
	campaignPath.concat("Campaign");
	ini.loadFileDirectory(AsciiString(campaignPath.str()), INI_LOAD_OVERWRITE, NULL);
}
