...
void TextTypeTransition::init( GameWindow *win )
{
	...
	m_frameLength = std::min(length, TEXTTYPETRANSITION_END);
}
...
void CountUpTransition::init( GameWindow *win )
{
	...
	if(m_intValue < COUNTUPTRANSITION_END)
	{
		...
		m_frameLength = std::min(m_intValue, COUNTUPTRANSITION_END);
	}
	else if(m_intValue/100 < COUNTUPTRANSITION_END)
	{
		m_countState = COUNT_100S;
		m_frameLength = std::min(m_intValue/100, COUNTUPTRANSITION_END);
	}
	else
	{
		m_countState = COUNT_1000S;
		m_frameLength = std::min(m_intValue/1000, COUNTUPTRANSITION_END);
	}
