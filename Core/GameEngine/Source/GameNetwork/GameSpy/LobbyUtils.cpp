	...
	UnsignedInt mouse)
{
	...
	{
		const char* start = room->getMap().reverseFindPathSeparator();
		if (start)
		{
			++start;
		}
...
static Int insertGame(GameWindow* win, GameSpyStagingRoom* game, Bool showMap)
{
	...
	if (showMap)
	{
		...
		{
			const char* start = game->getMap().reverseFindPathSeparator();
			if (start)
			{
				++start;
			}
...
void RefreshGameInfoListBox(GameWindow* mainWin, GameWindow* win)
{
	...
	//		else
	//		{
	//			const char *start = theRoom->getMap().reverseFindPathSeparator();
	//			if (start)
	//			{
	//				++start;
	//			}
