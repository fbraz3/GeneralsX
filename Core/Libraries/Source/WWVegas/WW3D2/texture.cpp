...
// 	// Instrumentation: sample first bytes of the surface to help debug format/byte-order
// 	{
// 		D3DLOCKED_RECT locked_rect;
// 		HRESULT lr = surface->LockRect(&locked_rect, NULL, D3DLOCK_READONLY);
// 		if (SUCCEEDED(lr) && locked_rect.pBits && locked_rect.Pitch > 0) {
// 			unsigned char *data = reinterpret_cast<unsigned char*>(locked_rect.pBits);
// 			int sample_bytes = 16;
// 			// Clamp to available data (single row)
// 			if ((int)locked_rect.Pitch < sample_bytes) sample_bytes = locked_rect.Pitch;
// 			printf("[TextureClass] Apply_New_Surface: name='%s' format=%d size=%ux%u pitch=%d first_%d_bytes=",
// 				Get_Texture_Name().c_str(), (int)TextureFormat, Width, Height, (int)locked_rect.Pitch, sample_bytes);
// 			for (int i=0;i<sample_bytes;i++) printf("%02x", data[i]);
// 			printf("\n");
// 			DX8_ErrorCode(surface->UnlockRect());
// 		} else {
// 			printf("[TextureClass] Apply_New_Surface: name='%s' format=%d size=%ux%u pitch=LOCK_FAIL\n",
// 				Get_Texture_Name().c_str(), (int)TextureFormat, Width, Height);
// 		}
// 	}
