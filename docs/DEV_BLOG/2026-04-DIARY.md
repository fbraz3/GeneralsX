...
## 2026-04-07 (SESSION 109): Validate and address Copilot findings from upstream sync PR

Reviewed `logs/copilot-findings.log` and applied only findings that were technically valid and aligned with GeneralsX cross-platform goals.

Applied fixes:
- `Core/GameEngine/Source/Common/System/Debug.cpp`
  - Removed `TheSystemIsUnicode` dependency in crash dialog path and used `MessageBoxW` directly on Windows.
- `Core/GameEngine/Source/Common/INI/INI.cpp`
  - Tightened integer `std::from_chars` parsing by requiring full token consumption.
- `Core/GameEngineDevice/Source/W3DDevice/GameClient/W3DTreeBuffer.cpp`
  - Restored defensive runtime guard for invalid/deleted tree type entries while keeping debug assertion.
- `Core/GameEngine/Source/GameNetwork/ConnectionManager.cpp`
  - Relaxed TGA transfer validation to accept structurally valid TGA files without requiring a TGA 2.0 footer.
- `Core/Libraries/Source/WWVegas/WWLib/TARGA.cpp`
  - Corrected TGA footer signature write to copy exactly 16 signature bytes.

Validation:
- macOS Zero Hour build passed:
  - `./scripts/build/macos/build-macos-zh.sh --build-only`
- macOS Generals build passed in local validation flow for this review cycle.

Notes:
- Rejected the specific Copilot suggestion to assert `sizeof(TGA2_SIGNATURE) == 16`; the string literal includes a terminator and the correct invariant for this code path is copying the 16-byte signature field.

## 2026-04-07 (SESSION 108): TheSuperHackers upstream refresh merge + macOS build validation

Completed a fresh upstream sync pass on branch `thesuperhackers-sync-04-07-2026` after updating to the latest `thesuperhackers/main` head.

What was changed:
- Merged latest upstream commits and resolved conflicts in CI workflow, INI parsing, crash/debug paths, registry compatibility code, DX8 wrapper globals, and ZH game engine startup code.
- Kept GeneralsX cross-platform architecture decisions during conflict resolution:
  - preserved current CI orchestration (`build-linux.yml`/`build-macos.yml`) and local change-filter contract.
  - preserved non-Windows-safe runtime behavior where upstream hunks were Win32-only.
  - preserved `GeneralsMD/Code/GameEngineDevice/Include/OpenALAudioManager.h` compatibility forwarder because it is still referenced by active source/CMake paths.
- Fixed post-merge compile regression by removing duplicated `toString(GameMode)` definitions from:
...
- macOS Zero Hour build passed:
  - `./scripts/build/macos/build-macos-zh.sh --build-only`
- macOS Generals build passed:
  - `./scripts/build/macos/build-macos-generals.sh --build-only`
- Linux build intentionally deferred to CI for this sync cycle.

Outcome:
- Branch now contains latest `thesuperhackers/main` (upstream ancestor check passes).
- PR opened: `https://github.com/fbraz3/GeneralsX/pull/75`
