...
## 2026-04-14 (SESSION 93): macOS Texture Loader — Stale Task Crash Elimination

**Objective**: Fix critical crash in headless replay/buildmapcache execution: `TextureLoadTaskClass::Begin_Compressed_Load() + 184` segfaulting at address `0x60`.

**Crash Analysis**:
- **Signature**: `EXC_BAD_ACCESS (SIGSEGV)` in `TextureLoadTaskClass::Begin_Compressed_Load()`
- **Address**: `0x60` (offset 96 decimal) — corresponds to `LockedSurfacePtr[3]` in task object structure
- **Context**: Crash occurs while initializing W3DLaserDraw texture during simulation frame 0 in `-headless -replay` and `-buildmapcache -headless` modes
- **Root Cause**: **Task pool recycling + async background loading race condition**
  - `TextureLoadTaskClass` objects are recycled from a free pool for performance
  - Background loader thread pops tasks and processes them asynchronously  
  - Stale task access occurs if: texture is destroyed prematurely, task is reused from pool while still referenced, or device shuts down during task execution
  - Dereferencing uninitialized/dangling `Texture` pointer → null access at task structure offsets

**Fix Applied** (commit `5fe874a70`):
1. **`Begin_Compressed_Load()`: Added guard**
   ```cpp
   if (!Texture || !DX8Wrapper::Is_Initted()) { return false; }
   ```
   Validates texture pointer and device readiness before file I/O and D3D texture creation.

2. **`Begin_Uncompressed_Load()`: Added same guard** — fallback path also needs protection.

3. **`Load()`: Added extended guard**
   ```cpp
   if (!Texture || !D3DTexture || !DX8Wrapper::Is_Initted()) { return false; }
   ```
   Protects mipmap loading phase where background thread is most active.

4. **`End_Load()`: Added texture validity check** — main thread applies surfaces; verify texture still exists.

**Validation Results**:
- ✅ **Replay execution**: Game progresses past texture crashes → new CRC mismatch errors (different issue, determinism-related)
- ✅ **Buildmapcache**: Runs significantly further (loads Shell Map, previously crashed at MapCache.ini transition)
- ✅ **No crash reports generated** post-patch (previous 11:39 crash report no longer reproduces)
- ✅ Build successful: `14M` binary, minor linker warnings (acceptable)

**Remaining Issues** (Lower Priority — unrelated to this crash):
- **Replay path resolution**: File paths being nested incorrectly (replay base dir treatment issue)
- **Buildmapcache hangs**: Progresses much further but doesn't complete or deterministically exit
- **CRC mismatch**: Frame 111 determinism divergence still present (underlying determinism issue)

**Design Lesson Learned**:
Object pool recycling in multithreaded systems requires defensive validity checks at task entry points. Background threads cannot reliably assume pooled objects remain valid between dequeue and execution.  Missing checks create race condition windows where garbage/stale pointers cause downstream segfaults that are hard to diagnose (address `0x60` didn't obviously point to a null dereference on first inspection).

**Next Actions**:
1. ~~Eliminate texture task crashes~~ → ✅ COMPLETE
2. Investigate buildmapcache hang/incomplete execution (separate investigation)
3. Analyze CRC mismatch at frame 111 determinsim blocker
4. Prepare Linux validation validation post-macOS stabilization
