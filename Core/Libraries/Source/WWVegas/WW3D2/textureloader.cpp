...
void TextureLoader::Update(void(*network_callback)(void))
{
	...
	if (update_count < 5) {
		printf("[Phase 54] TextureLoader::Update - START count=%d, callback=%p\n", update_count, (void*)network_callback);
	...
	if (update_count < 5) {
		printf("[Phase 54] TextureLoader::Update - Processing deferred tasks\n");
	...
	if (update_count < 5) {
		printf("[Phase 54] TextureLoader::Update - Processing finished tasks\n");
	...
	if (update_count < 5) {
		printf("[Phase 54] TextureLoader::Update - Processing thumbnail tasks\n");
	...
	if (update_count < 5) {
		printf("[Phase 54] TextureLoader::Update - Processing delete list\n");
...
//		delete task;
	...
	if (update_count < 5) {
		printf("[Phase 54] TextureLoader::Update - DONE count=%d\n", update_count);
...
{
	printf("[TextureLoadTask::Begin_Texture_Load] ENTER for texture: %s\n", 
	...
	if (ThreadClass::_Get_Current_Thread_ID()==DX8Wrapper::_Get_Main_Thread_ID()) {
		printf("[TextureLoadTask::Begin_Texture_Load] In main thread\n"); 

		bool loaded=false;
		if (Texture->Is_Compression_Allowed()) {
			printf("[TextureLoadTask::Begin_Texture_Load] Compression allowed, trying DDS...\n"); 
			DDSFileClass dds_file(Texture->Get_Full_Path(),Get_Reduction());
			printf("[TextureLoadTask::Begin_Texture_Load] DDS file created, checking availability...\n"); 
			if (dds_file.Is_Available()) {
				...
				printf("[TextureLoadTask::Begin_Texture_Load] Creating DX8 texture %ux%u, format=%d, mips=%u\n", Width, Height, Format, mip_level_count); 
				D3DTexture=DX8Wrapper::_Create_DX8_Texture(Width,Height,Format,(MipCountType)mip_level_count);
				printf("[TextureLoadTask::Begin_Texture_Load] DX8 texture created: %p\n", D3DTexture); 
		...
		if (!loaded) {
			printf("[TextureLoadTask::Begin_Texture_Load] DDS not loaded, trying TGA...\n"); 
			Targa targa;
			printf("[TextureLoadTask::Begin_Texture_Load] Opening TGA: %s\n", Texture->Get_Full_Path() ? Texture->Get_Full_Path().str() : "NULL"); 
			if (TARGA_ERROR_HANDLER(targa.Open(Texture->Get_Full_Path(), TGA_READMODE),Texture->Get_Full_Path())) {
				printf("[TextureLoadTask::Begin_Texture_Load] TGA open FAILED, using missing texture\n"); 
