...
#else
		DEBUG_LOG(("Failed to set working directory to '%s'", path));
#endif
		return FALSE;
	}
...
#else
	if (!GetModuleFileName(nullptr, buffer, ARRAY_SIZE(buffer)))
	{
		DEBUG_LOG(("Failed to get executable path for working directory"));
		return FALSE;
	}
#endif
...
#ifdef _WIN32
	Char *pEndBackslash = strrchr(buffer, '\\');
	if (pEndBackslash > pEnd)
		pEnd = pEndBackslash;
#endif
	if (pEnd == nullptr)
	{
