...
{
	INI ini;
	AsciiString challengePath;
	challengePath.concat("Data");
	challengePath.concat(GET_PATH_SEPARATOR());
	challengePath.concat("INI");
	challengePath.concat(GET_PATH_SEPARATOR());
	challengePath.concat("ChallengeMode");
	ini.loadFileDirectory(AsciiString(challengePath.str()), INI_LOAD_OVERWRITE, NULL);
}
