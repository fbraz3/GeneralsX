...
**Root Cause**: In session 72, `macos-vulkan` preset was changed from `inherits: "default-vcpkg"` to `inherits: "default"` to remove the broken `$env{VCPKG_ROOT}` reference (VCPKG_ROOT was unset in CI). This removed the vcpkg toolchain file entirely, which broke resolution of `gli`, `glm`, `freetype`, `fontconfig` — all defined in `vcpkg.json`. Locally they still worked because `VCPKG_ROOT=$HOME/vcpkg` was already set in the shell env.
...
#### Problem Found
Game executed from `~/GeneralsX/GeneralsMD/` with correct assets, but:
```
...
**Command**:
```bash
cd ~/GeneralsX/GeneralsMD && ./run.sh -win
```
...
**Files involved**:
- `~/GeneralsX/GeneralsMD/run.sh` ← Wrapper script (key fix!)
