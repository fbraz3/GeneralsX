...
void CreditsManager::load(void)
{
	...
	AsciiString creditsPath;
	creditsPath.concat("Data");
	creditsPath.concat(GET_PATH_SEPARATOR());
	creditsPath.concat("INI");
	creditsPath.concat(GET_PATH_SEPARATOR());
	creditsPath.concat("Credits");
	ini.loadFileDirectory(AsciiString(creditsPath.str()), INI_LOAD_OVERWRITE, NULL);
