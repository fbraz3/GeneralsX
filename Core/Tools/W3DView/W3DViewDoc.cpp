...
CW3DViewDoc::LoadAssetsFromFile(LPCTSTR lpszPathName)
{
	...
	//
	if (::strrchr(lpszPathName, GET_PATH_SEPARATOR()[0])) {
		CString stringTemp = lpszPathName;
		stringTemp = stringTemp.Left((long)::strrchr(lpszPathName, GET_PATH_SEPARATOR()[0]) - (long)lpszPathName);
...
)
{
		...
		(m_pCScene != NULL))
	{
		...
		if (::strrchr(pszFilename, GET_PATH_SEPARATOR()[0]) == NULL)
		{
			// Add the current directories path to the filename
			TCHAR szPath[MAX_PATH] = { 0 };
			::GetCurrentDirectory(sizeof(szPath), szPath);

			if (szPath[::lstrlen(szPath) - 1] != GET_PATH_SEPARATOR())
			{
				// Ensure the path is directory delimited
				strlcat(szPath, GET_PATH_SEPARATOR(), ARRAY_SIZE(szPath));
			}
...
CW3DViewDoc::Make_Movie(void)
{
	...
	if (m_pCRenderObj && m_pCAnimation) {
		...
		LPTSTR ppath = ::strrchr(filename, GET_PATH_SEPARATOR()[0]);
		if (ppath != NULL) {
			ppath[0] = 0;
		}
