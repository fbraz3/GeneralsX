...
{
	// Normalize forward slashes to backslashes to match archive internal path storage
	AsciiString normalizedFilename(filename);
	char* buf = const_cast<char*>(normalizedFilename.str());
	for (int i = 0; buf[i] != '\0'; i++) {
		if (buf[i] == '/') buf[i] = '\\';
	}

	// printf("[StdBIGFile::openFile] Original: '%s', Normalized: '%s'\n", filename, normalizedFilename.str());
	// 

	const ArchivedFileInfo* fileInfo = getArchivedFileInfo(normalizedFilename);

	if (fileInfo == NULL) {
		// printf("[StdBIGFile::openFile] getArchivedFileInfo returned NULL for '%s'\n", normalizedFilename.str());
		// 
		return NULL;
	}

	// printf("[StdBIGFile::openFile] SUCCESS - found file info, offset=%u, size=%u\n", fileInfo->m_offset, fileInfo->m_size);
	// 
