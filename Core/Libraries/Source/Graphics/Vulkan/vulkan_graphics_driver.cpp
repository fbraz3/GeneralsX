  ...
  public:
    ...
    bool Create(bool debug_mode) {
      ...
      // SDL_Vulkan_GetInstanceExtensions handles X11/Wayland/etc. automatically
      {
        ...
        else if (sdl_ext_count > 0) {
          std::vector<const char*> sdl_extensions(sdl_ext_count);
          if (SDL_Vulkan_GetInstanceExtensions(NULL, &sdl_ext_count, sdl_extensions.data())) {
            for (unsigned int i = 0; i < sdl_ext_count; i++) {
