          ...
          continue;
			...
			if ( drw )
			{
				...
				if (tmp)
				{
					...
					UnsignedInt emitterCount = std::max(15, static_cast<int>(REAL_TO_INT_CEIL(data->m_sparksPerCubicFoot * victimVolume)));
					...
					for (UnsignedInt e = 0 ; e < emitterCount; ++e)
					{
						...
						if (sys)
						{
							...
							sys->setSystemLifetime(std::max(static_cast<unsigned int>(0), data->m_disabledDuration - 30));
