...
## 31/07/2026
### Threading & Platform Timers
- Fixed a 64-bit portability bug in `TIMEGETTIME` (`systimer.h`) where returning `unsigned long` on Linux (64-bit) caused extreme truncation when stored in 32-bit `unsigned` variables. This resolved an immediate timeout bug in `ThreadClass::Stop` that was erroneously triggering `pthread_cancel` and causing a segfault (`exit 139`) during headless replay test teardowns on CI.
