	...
	WindowMsgData mData1, WindowMsgData mData2)
{
	...
	case GCM_UPDATE_TEXT:
	{
		...
		{
			trimmedNick.trimEnd(LGET_PATH_SEPARATOR());
			trimmedNick.trimEnd(LGET_PATH_SEPARATOR());
		}
		if (!trimmedEmail.isEmpty())
		{
			trimmedEmail.trimEnd(LGET_PATH_SEPARATOR());
			trimmedEmail.trimEnd(LGET_PATH_SEPARATOR());
		}
	...
	case GBM_SELECTED:
	{
		...
		else if (controlID == buttonTOSID)
		{
			...
			else
			{
				...
				fileName.concat(GET_PATH_SEPARATOR());
				fileName.concat(GetRegistryLanguage());
				fileName.concat(GET_PATH_SEPARATOR());
				fileName.concat("TOS.txt");
